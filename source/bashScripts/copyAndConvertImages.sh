#!/bin/bash
# This bash script copies and converts .jpg images from on directory to wsq images in another directory.
# In addition, each image copied is converted from a .jpg image format to a 
# .wsq image format. This is example code, it currently will not work on the 
# server.
...
then
    echo "Error in copyAndConvertImages.sh"
...
do
    ...
    ./bin/cnvrtIm "$inputDirectory$filename" "$outputDirectory$rootFilename$jpgExt"
